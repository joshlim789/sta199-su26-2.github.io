...
TLDR: The testing/training split is random, but we want the results to be reproducible, so we "freeze the random numbers in time" by setting a seed.
If we don't tell you exactly what seed to use on an assignment, you can pick any positive integer you want.
...
- Write down the number of TV episodes you have watched in the past week
- Folder your piece of paper in half
- Enter your name into the reaping

# Logistic wrap-up (Review)
...
::: callout-note
We didn't interpret the intercept $b_0=1.57$ directly.
We transformed it into an interpretable quantity using the *probability* version of the logsitic regression model.
:::
...
Don't waste time fitting a model.
Just flip a coin:
